...
class ThriftParser(
  ...
  )(implicit val logger:Logger = LoggerFactory.getLogger(classOf[ThriftParser])) extends RegexParsers {
  ...
  lazy val function = (opt(comments) ~ (opt("oneway") ~ functionType)) ~ (simpleID <~ "(") ~ (rep(field) <~ ")") ~
    ...
    case comment ~ (oneway ~ ftype) ~ id ~ args ~ exceptions =>
      Function(
        id,
        id.name,
        if (oneway.isDefined) OnewayVoid else ftype,
        fixFieldIds(args),
        exceptions.map(fixFieldIds).getOrElse(Nil),
        comment)
  }
  ...
  lazy val const = opt(comments) ~ ("const" ~> fieldType) ~ simpleID ~ ("=" ~> rhs) ~ opt(listSeparator) ^^ {
    case comment ~ ftype ~ sid ~ constPart ~ _ =>
      ConstDef(sid, ftype, convertRhs(ftype, constPart), comment)
  }
  ...
  lazy val enum = (opt(comments) ~ (("enum" ~> simpleID) <~ "{")) ~ rep(opt(comments) ~ simpleID ~ opt("=" ~> intConstant) <~
    ...
    case comment ~ sid ~ items ~ annotationPart  =>
      var failed: Option[Int] = None
      val seen = new mutable.HashSet[Int]
      var nextValue = 0
      val values = new mutable.ListBuffer[EnumFieldDef]
      items.foreach {
        case c ~ k ~ v =>
          val value = v.map {
            _.value.toInt
          ...
          nextValue = value + 1
          seen += value
          values += EnumFieldDef(k, value, c)
      }
      if (failed.isDefined) {
        throw new RepeatingEnumValueException(sid.name, failed.get)
      } else {
        EnumDef(sid, values.toList, comment, annotationPart)
      }
  }
  ...
  lazy val struct = positioned(structLike("struct") ^^ {
    case comment ~ sid ~ fields ~ annotations =>
      StructDef(sid, sid.name, fixFieldIds(fields), comment, annotations)
  })
