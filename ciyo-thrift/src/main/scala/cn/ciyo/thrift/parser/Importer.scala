...
trait Importer extends (String => Option[FileContents]) {
  private[thrift] def canonicalPaths: Seq[String] // for tests
   ...
   */
  private[thrift] def getResolvedPath(filename: String): Option[String] = None
}
...
case class DirImporter(dir: File) extends Importer {
  private[thrift] def canonicalPaths = Seq(dir.getCanonicalPath) // for test
  ...
  override private[thrift] def getResolvedPath(filename: String): Option[String] = {
