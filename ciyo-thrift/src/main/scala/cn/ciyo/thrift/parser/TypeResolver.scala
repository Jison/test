...
case class ResolvedDocument(document: Document, resolver: TypeResolver) {
   ...
   */
  def collectParentServices(service: ServiceDef): Seq[(ResolvedDocument, ServiceDef)] = {
 ...
 *
  ...
  def resolveParentServices(
    ...
    val resolvedServices: Seq[(ResolvedDocument, ServiceDef)] = collectParentServices(service)
...
case class ResolvedService(serviceID: Identifier, service: ServiceDef)
...
case class TypeResolver(
                         ...
                         constMap: Map[String, ConstDef] = Map.empty,
                         serviceMap: Map[String, ServiceDef] = Map.empty,
  ...
  def resolveServiceParent(parent: ServiceParent): ServiceDef =
  ...
  def resolveService(sid: SimpleID): ServiceDef = serviceMap.getOrElse(sid.name,
   ...
   */
  def withConst(const: ConstDef): TypeResolver = {
   ...
   */
  def withService(service: ServiceDef): TypeResolver = {
  ...
  def apply(doc: Document, scopePrefix: Option[SimpleID] = None): ResolvedDocument = {
    ...
    ResolvedDocument(doc.copy(defs = defBuf), resolver)
  }
  ...
  def apply(definition: Definition, scopePrefix: Option[SimpleID]): ResolvedDefinition = {
    definition match {
      ...
      case s@ServiceDef(sid, parent, fs, _) =>
      ...
      case e @ EnumDef(sid, _, _, _) =>
        ResolvedDefinition(e, withType(sid.name, EnumType(e, scopePrefix)))
      case s @ Senum(sid, _) =>
        ResolvedDefinition(s, withType(sid.name, TString))
      case d: EnumFieldDef => ResolvedDefinition(d, this)
      case d: FunctionArgsDef => ResolvedDefinition(d, this)
      case d: FunctionResultDef => ResolvedDefinition(d, this)
    }
  }
  ...
  def apply(c: RHS, fieldType: FieldType): RHS = c match {
    ...
    case m @ MapRHS(elems) =>
      fieldType match {
        ...
        case st @ StructType(structLike: StructLikeDef, _) =>
