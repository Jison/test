...
case class StructType(struct: StructLikeDef, scopePrefix: Option[SimpleID] = None) extends NamedType {
  val sid = struct.sid
  override def toString: String = "Struct(?)"
}

case class EnumType(enum: EnumDef, scopePrefix: Option[SimpleID] = None) extends NamedType {
  val sid = enum.sid
