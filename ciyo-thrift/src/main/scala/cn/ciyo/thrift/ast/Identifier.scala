...
sealed abstract class Identifier extends IdNode {
  ...
  private[thrift] def +(str: String): String =
    throw new UnsupportedOperationException("do not use \"+\" operation on Identifiers")
}
...
object Identifier {
  ...
  def toTitleCase(str: String): String = toCamelCase(str, firstCharUp = true)
  ...
  def toCamelCase(str: String, firstCharUp: Boolean = false): String = {
    str.takeWhile(_ == '_') + str.
      ...
      zipWithIndex.map { case (part, ind) =>
        ...
        new mutable.StringBuilder(part.length).append(first).append(rest)
      }.
      mkString
  }
}
