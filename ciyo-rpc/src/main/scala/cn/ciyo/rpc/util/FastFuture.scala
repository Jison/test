...
class FastFuture[A](val future: Future[A]) extends AnyVal {
	...
	def filter(pred: A ⇒ Boolean)(implicit executor: ExecutionContext): Future[A] =
		flatMap { r ⇒
			if (pred(r)) future
			else throw new FastFuture.NoSuchElementException("Future.filter predicate is not satisfied")
		}
...
object FastFuture {
	class NoSuchElementException(msg: String) extends scala.NoSuchElementException(msg) with NoStackTrace
